...
function onOpen() {
  SpreadsheetApp.getUi().createMenu("AIMAX 발송")
    .addItem("0. 실행 계정/한도 확인", "checkAimaxQuota")
...
function checkAimaxQuota() {
  var activeUser = Session.getActiveUser().getEmail() || "(확인 불가)";
  var effectiveUser = Session.getEffectiveUser().getEmail() || "(확인 불가)";
  var quota = MailApp.getRemainingDailyQuota();
  SpreadsheetApp.getUi().alert(
    "실행 계정(active): " + activeUser +
    "\n권한 계정(effective): " + effectiveUser +
    "\n오늘 남은 Apps Script 이메일 수신자 한도: " + quota + "명"
  );
}
...
function sendAimaxTestToMe() {
  ...
  if (MailApp.getRemainingDailyQuota() < 1) return SpreadsheetApp.getUi().alert("오늘 발송 한도가 0명입니다. 24시간 뒤 다시 실행해주세요.");
  var item = readItem(sheet, row, h), me = Session.getActiveUser().getEmail();
  MailApp.sendEmail({to: me, subject: "[테스트] " + item.subject, body: plainBody(item) + "\n\n---\n실제 수신자: " + item.to, htmlBody: htmlBody(item)});
...
function sendPendingAimaxEmails() {
  ...
  var quota = MailApp.getRemainingDailyQuota();
  if (quota < 1) return SpreadsheetApp.getUi().alert("오늘 남은 발송 한도가 0명입니다. 24시간 뒤 다시 실행해주세요.");
  for (var row = 2; row <= sheet.getLastRow(); row++) {
    if (sent >= MAX_SEND_PER_RUN || sent >= quota) break;
    ...
    try {
      MailApp.sendEmail({to: item.to, subject: item.subject, body: plainBody(item), htmlBody: htmlBody(item)});
      ...
      if (String(e.message || e).indexOf("too many times") >= 0) break;
    }
  }
  SpreadsheetApp.getUi().alert("발송 " + sent + "건\n건너뜀 " + skipped + "건\n실패 " + failed + "건\n남은 한도 " + MailApp.getRemainingDailyQuota() + "명");
}
...
function htmlBody(item) {
  ...
  return '<div style="font-family:Arial,Apple SD Gothic Neo,Malgun Gothic,sans-serif;font-size:15px;line-height:1.75;color:#222;max-width:640px;">' +
    ...
    '<p style="margin:0 0 8px;"><strong>3. 이메일:</strong> ' + email + '</p><p style="margin:0 0 8px;"><strong>4. 구매 상품:</strong> ' + product + '</p>' +
    ...
    '<p style="margin:0;color:#555;">설치가 막히면 웹앱의 권한 허용 가이드를 순서대로 확인해주세요.</p></div>';
}

function getSheet() { return SpreadsheetApp.getActiveSpreadsheet().getSheets()[0]; }
function ensureHeaders(sheet) {
  var h = headerMap(sheet); need(h, "to"); need(h, "subject"); need(h, "setup_url");
...
function headerMap(sheet) {
  ...
  for (var i = 0; i < vals.length; i++) { var key = String(vals[i] || "").trim(); if (key) h[key] = i + 1; }
  return h;
}
function need(h, key) { if (!h[key]) throw new Error("필수 열이 없습니다: " + key); }
function addHeader(sheet, name) { var col = sheet.getLastColumn() + 1; sheet.getRange(1, col).setValue(name); return col; }
...
function cell(sheet, row, col) { if (!col) return ""; return String(sheet.getRange(row, col).getValue() || "").trim(); }
function writeResult(sheet, row, h, status, sentAt, error) {
  sheet.getRange(row, h.send_status).setValue(status); sheet.getRange(row, h.sent_at).setValue(sentAt || ""); sheet.getRange(row, h.send_error).setValue(error || "");
}
