...
assert(productCatalog.some((product) => product.product === "semu" && product.job_kinds.includes("semu_tax_invoice")), "semu_product_job_kind_missing");
...
const semu = publicWorkers.find((worker) => worker.staff_code === "semu");
assert(semu?.name === "세무 직원", "semu_worker_name_missing");
assert(semu?.role === "전자세금계산서 발행 직원", "semu_worker_role_missing");
assert(semu?.execution === "web_module" && semu?.type === "web_module", "semu_execution_contract_missing");
assert(semu?.module_key === "tax", "semu_module_key_missing");
assert(semu?.profile_image === "/assets/avatar_placeholder.svg", "semu_placeholder_profile_missing");
assert(__catalogTest.productList("bundle").includes("semu"), "bundle_should_include_semu_product");
...
assert(__catalogTest.isJobAllowed(bundleOnlyUser, "semu_tax_invoice"), "bundle_should_allow_semu_tax_invoice");
...
assert(appHtml.includes('id="taxJobForm"'), "semu_tax_job_form_missing");
assert(appHtml.includes("테스트베드 — 실제 국세청 전송 아님"), "semu_testbed_label_missing");
