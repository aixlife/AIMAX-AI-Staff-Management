...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "aimax-runner-start-timeout-smoke-"));
const port = 20900 + Math.floor(Math.random() * 500);
const baseUrl = `http://127.0.0.1:${port}`;
const email = "runner-start-timeout-smoke@example.test";
...
writeJson(path.join(tmpDir, "users.json"), {
  version: 1,
  users: [{
    id: "runner-start-timeout-smoke-user-id",
    email,
    name: "Runner Start Timeout Smoke",
    status: "active",
...
const child = childProcess.spawn(process.execPath, ["oracle/aimax-reports-api/server.js"], {
  ...
  env: {
    ...
    AIMAX_AGENT_JOB_START_TIMEOUT_SECONDS: "1",
...
async function claimJob(auth) {
  const claimed = await request("/api/agent/next-job?platform=windows&device_label=Smoke%20Windows", {
    headers: auth,
  });
...
try {
  ...
  const timeoutJobId = await createJob(auth, "runner timeout");
  assert(await claimJob(auth) === timeoutJobId, "timeout_job_claim_mismatch");
  await request("/api/agent/jobs/update", {
    ...
    body: JSON.stringify({
      job_id: timeoutJobId,
      status: "running",
      log: "local queue accepted",
      result: { runner_event: "claimed" },
    }),
  });
  await new Promise((resolve) => setTimeout(resolve, 1200));
  await heartbeat(auth);
  const timedOut = await getJob(auth, timeoutJobId);
  assert(timedOut.status === "failed", `expected_runner_timeout_failed:${JSON.stringify(timedOut)}`);
  assert(timedOut.failed_stage === "runner_start_timeout", `expected_runner_start_stage:${JSON.stringify(timedOut)}`);

  const startedJobId = await createJob(auth, "runner started");
  assert(await claimJob(auth) === startedJobId, "started_job_claim_mismatch");
  await request("/api/agent/jobs/update", {
    method: "POST",
    headers: auth,
    body: JSON.stringify({
  ...
  await new Promise((resolve) => setTimeout(resolve, 1200));
  await heartbeat(auth);
  const stillRunning = await getJob(auth, startedJobId);
  assert(stillRunning.status === "running", `started_job_should_remain_running:${JSON.stringify(stillRunning)}`);

  console.log("RUNNER_START_TIMEOUT_SMOKE_OK");
} finally {
  child.kill("SIGTERM");
  try {
