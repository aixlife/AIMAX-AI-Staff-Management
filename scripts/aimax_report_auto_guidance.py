...
GUIDANCE: dict[str, Guidance] = {
    ...
    "core_exe_missing": Guidance(
        ...
        status_label="사용자 확인 필요",
        public_message=(
            "AIMAX 프로그램 본체가 회원님 컴퓨터에서 없어진 상태입니다. 바로가기는 남아 있어서 아이콘은 눌리지만, "
            "실행될 프로그램이 없어 창이 뜨지 않습니다. 저희 쪽 설치 파일은 확인해보니 정상이고, "
            "회원님 계정이나 설정 문제도 아닙니다. 백신 프로그램이 파일을 지웠거나 설치가 끝까지 되지 않았을 때 "
            "이런 상태가 됩니다. 어느 쪽인지는 아래 한 가지만 확인해주시면 바로 알 수 있습니다."
        ),
        next_update_message=(
            "먼저 프로그램이 정말 없어졌는지만 확인해주세요. 키보드 윈도우키를 누른 채 R을 누르면 작은 창이 뜹니다. "
            "거기에 %LOCALAPPDATA%\\Programs\\AIMAX 를 붙여넣고 확인을 누르면 AIMAX 설치 폴더가 열립니다. "
            "그 안에 AIMAX 실행 파일이 보이면 원인이 다른 데 있으니 그대로 알려주세요. "
            "보이지 않으면 쓰시는 백신(윈도우 보안, V3, 알약 등)에서 차단하거나 지운 기록에 AIMAX 가 있는지 보고, "
            "있으면 '허용'으로 되돌린 뒤 설치 파일을 한 번 더 실행해주세요. "
            "어느 단계든 막히면 그 화면을 캡처해 '아직 안 돼요'로 보내주시면 저희가 이어서 안내드립니다."
        ),
    ),
