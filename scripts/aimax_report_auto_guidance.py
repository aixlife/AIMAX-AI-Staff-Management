...
GUIDANCE: dict[str, Guidance] = {
    ...
    "core_exe_missing": Guidance(
        ...
        status_label="사용자 확인 필요",
        public_message=(
            "AIMAX 프로그램 본체가 컴퓨터에서 없어진 상태입니다. 바로가기는 남아 있어서 아이콘은 눌리지만, "
            "실행될 프로그램이 없어 창이 뜨지 않습니다. 다시 설치해도 같은 창이 뜬다면 백신 프로그램이 "
            "설치 직후 그 파일을 지웠을 가능성이 큽니다. 회원님 계정이나 설정 문제는 아닙니다."
        ),
        next_update_message=(
            ...
            "그 안에 AIMAX 실행 파일이 보이지 않으면 백신이 지운 것이 맞습니다. "
            "쓰시는 백신(윈도우 보안, V3, 알약 등)에서 차단하거나 지운 기록을 열어 AIMAX 를 '허용'으로 되돌린 뒤 "
            "설치 파일을 한 번 더 실행해주세요. "
