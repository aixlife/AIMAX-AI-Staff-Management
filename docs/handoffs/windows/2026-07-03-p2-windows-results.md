...
검증: 패치된 프로덕션 함수를 라이브 호출해 **3회 연속 삽입 성공**(카운트 0→1→2→3, `blogfiles.pstatic.net` 업로드 URL 확인). 합성 ClipboardEvent(untrusted) 방식은 에디터가 무시함을 확인하고 기각. 실사용 e2e(서버 잡 경유) 최종 확인은 CEO 재실행 대기.
