...
## 추가 — 이미지 삽입 실패 근본 수정 (2026-07-03 오후, 커밋 dddd1a8)

CEO 실작업(likimartin, 12:00 KST)에서 "이미지 생성 성공 → 에디터 미반영(1→1)" 재현 후, 실제 네이버 에디터 라이브 실험으로 3중 원인 확정:
1. `_editor_image_node_count`가 툴바 사진 버튼(`se-image-toolbar-button`)을 오검출 — 빈 에디터에서 before=1 (팬텀 카운트)
2. `execute_script click()`은 SmartEditor ONE의 키 입력 버퍼(iframe)로 포커스를 못 넘겨 Ctrl+V가 에디터 밖으로 샘
3. "업로드 성공" 로그가 검증 없이 3초 후 무조건 출력

수정(`posting/editor.py`): 실제 업로드 img(http/blob/data)만 카운트 + ActionChains 실클릭·캐럿 설정 후 실키 Ctrl/Cmd+V + 성공 판정을 DOM 검증으로 일원화. 클립보드를 주경로, 이미지 버튼을 검증 폴백으로 재배치(현 에디터는 버튼 클릭 시 접근 가능한 file input을 만들지 않음 — 전 프레임 0개 라이브 확인). 진단 payload 계약·맥 분기 유지.

검증: 패치된 프로덕션 함수를 라이브 호출해 **3회 연속 삽입 성공**(카운트 0→1→2→3, `blogfiles.pstatic.net` 업로드 URL 확인). 합성 ClipboardEvent(untrusted) 방식은 에디터가 무시함을 확인하고 기각.

**실사용 e2e 최종 확인 (13:34~13:40 KST, CEO 실행, 미션 4 완결)**: demo 계정 서버 생성 글쓰기 잡 → 수정판 v1.0.56 러너가 수신 → 네이버 로그인 → 이미지 2장 생성·삽입 **모두 실제 반영**(0→1, 1→2 검증 통과) → 임시저장 완료. 동일 유형 작업이 v1.0.55에서 실패(12:00, 1→1)했던 것과 대조 확정. 미션 4의 신규 설치→로그인→서버 생성 글쓰기→로컬 저장→업데이트 재설치→재실행 사이클 전부 실기 통과(발행은 테스트 계정 정책에 따라 임시저장 모드).
