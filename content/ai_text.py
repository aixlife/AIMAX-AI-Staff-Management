...
def generate_blog_content(keyword, api_key, style_id="info", model="gemini-3.6-flash",
...
def _normalize_gemini_model_id(model):
    ...
    # 2026-07-22: 은퇴/과부하 모델은 성격을 유지해 후속으로 넘긴다. flash 는 3.6-flash(기본값),
    # lite 계열은 저렴한 3.5-flash-lite 로. 명시적 3.1 Pro 선택만 유료 프리뷰 유지
    # (app.py _LEGACY_AI_MODEL_MAP 과 일치).
    ...
    aliases = {
        ...
        "gemini-2.5-flash-lite": lite_model,
        "gemini-3.1-flash-lite": lite_model,
...
def _generate_with_gemini(prompt, api_key, model_id="gemini-3.6-flash"):
