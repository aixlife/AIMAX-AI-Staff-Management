...
| 1 관측성 | 실패를 데이터로 + **감시 지표를 리포트에서 실패로 교체** | 6-3(a)(b) 정형 결과·단계 이벤트, 6-5(a) 잡 타임아웃, 빌드 커밋 스탬프, **감시 지표 5종**(잡 실패율 급증 · 동일 error/stage 급증 · 특정 워커 실패 0 지속 · 플릿 버전 정체 · 카탈로그-번들 불일치) | `posting/result.py`, `diagnostics/events.py`, result 스키마 문서, 서버 규칙 매핑, **지표 산출 스크립트 + 알림 규칙** | 실패 유도 스모크에서 서버 잡 `result.detail_code`·`stage` 채워짐, 기존 스모크 회귀 0, 타임아웃 경로 PASS, **과거 사고 3건(로그인 파손·현주 무성공·미배달)을 지표가 소급 탐지하는지 실데이터 검증** | 3~4 |
...
합계(0 제외, 6 제외): 10~14 라운드. 게이트는 날짜가 아니라 이벤트(각 Phase 검증 PASS + CEO 확인). 사람 대기 구간: Phase 0 실기 로그인(사용자 계정), Phase 3 실발행 승인, Phase 4 카나리 사용자 선정.

---

### 구조 팩 v1 유료 실측 결과 (2026-08-18, 1회)

CEO 승인 아래 실제 모델로 1회만 생성해 구조 준수를 확인했다. 발행하지 않았고 재시도도 없다.

| 항목 | 값 |
|---|---|
| 제공자·모델 | Gemini `gemini-3.6-flash` (프로덕션 기본값과 동일) |
| 토큰 | 입력 673 / 출력 2,909 |
| **실비용** | **$0.0228 (약 32원)** |
| 계획 항목 수 | 6 |
| 실제 소제목 수 | 6 |
...
계획한 뼈대와 실제로 나온 소제목이 1:1로 맞았다.

| 계획 블록 | 실제 소제목 |
|---|---|
| 질문으로 열기 | 집에서 만든 디저트, 어떻게 합법적으로 판매하고 주문받을까? |
| 핵심 정리 | 홈베이킹 판매의 출발점, 이것만은 꼭 기억하세요 |
| 단계별 순서 | 차근차근 따라 하는 홈베이킹 주문 접수 4단계 |
| 흔한 실수 | 초보자가 빠지기 쉬운 주문 접수 실수와 예방법 |
| 비용·기간 감각 | 준비 기간과 소요 비용의 현실적인 범위 |
| 요약으로 닫기 | 성공적인 첫 주문을 위한 핵심 요약 |

본문 품질도 확인했다. **없는 수치를 지어내지 않았다** — 비용 섹션에서 정확한 금액 대신
"최소 2주에서 한 달", "공유주방 대여 시 이용료가 추가 변수"처럼 범위와 변수로 썼다.
프롬프트의 "확인되지 않은 통계·가격·법적 단정 금지" 규칙이 구조 지시와 충돌하지 않고 함께 지켜졌다.

재현: `AIMAX_LIVE_PAID_OK=1 node scripts/live_yeri_structure_generation.mjs`
(플래그 없이는 실행 거부. 실패 시 재시도 없이 종료코드 1.)

### Phase 1 보강 (2026-08-18 CEO 결정)

감시 지표 교체를 Phase 1에 붙인다. 근거는 별도 진단서
`claudedocs/auto-repair-automation-diagnosis-20260818.md` 참조 — 요약하면
**지금 감시가 보는 신호("사람이 보낸 오류 보고")로는 오늘 사고 3건 중 하나도 잡히지 않는다.**

검증 기준을 하나 더 붙였다: 새 지표가 **과거 사고를 소급해서 탐지하는지** 실데이터로 확인한다.
지표를 새로 만들 때 "앞으로 잘 잡히겠지"로 끝내면 다음에도 같은 걸 놓친다.
서버에 이미 6~8월 잡 데이터가 있으므로 소급 검증이 가능하다.
