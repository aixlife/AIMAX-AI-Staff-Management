...
const WORKERS = {
  ...
  max_alert: {
    ...
    releaseUrl: "",
    setupDownloadUrl: "https://github.com/makefriendscoltd-design/maxalert-releases/releases/download/v0.2.4/MaxAlert-Setup-0.2.4.exe",
    portableDownloadUrl: "",
    downloadLabel: "설치 파일 다운로드",
    supportedPlatforms: ["windows", "macos"],
    version: "0.2.4",
    ...
    executionOptions: [
      {
        ...
        platforms: ["windows"],
        status: "available",
        url: "https://github.com/makefriendscoltd-design/maxalert-releases/releases/download/v0.2.4/MaxAlert-Setup-0.2.4.exe",
        primary: true,
        description: "Windows용 맥스 v0.2.4 설치 파일입니다 (Tauri 통합, 16.6MB). 노션 일정에 시간을 넣어도 오늘 목록에 그대로 남고, 포스트잇 아래 레벨 캐릭터를 설정에서 끌 수 있습니다.",
      },
      {
        kind: "mac_download",
        label: "Apple Silicon Mac 앱 다운로드",
        platforms: ["macos"],
        status: "available",
        url: "https://github.com/makefriendscoltd-design/maxalert-releases/releases/download/v0.2.4/MaxAlert_0.2.4_aarch64.dmg",
        primary: false,
        description: "Apple Silicon Mac용 Tauri v0.2.4 DMG입니다. 첫 실행에서 '열 수 없습니다' 안내가 뜨면 시스템 설정 > 개인정보 보호 및 보안 맨 아래의 '그래도 열기'를 눌러 실행해주세요.",
      },
    ],
  },
  eunseo: {
...
const PUBLIC_DOWNLOAD_FILES = new Set([
  ...
  "AIMAX-Office-Manager-macOS-0.2.1-aarch64.dmg",
  "AIMAX-Office-Manager-portable.exe",
  "Pencil-Setup-1.0.0.exe",
  "Pencil-portable.exe",
  "MaxAlert-Setup-0.2.4.exe",
  "MaxAlert_0.2.4_aarch64.dmg",
  "MaxAlert-Setup-0.2.3.exe",
  "MaxAlert_0.2.3_aarch64.dmg",
  "MaxAlert-Setup-0.2.2.exe",
  "MaxAlert_0.2.2_aarch64.dmg",
]);
...
function buildYeriGenerationPrompt(payload) {
  ...
  return [
    ...
    // ── 읽는 사람 기준 규칙 (2026-08-26 실측 근거) ──
    // 그전까지 이 프롬프트에는 글자 수·이미지 개수·키워드 지시만 있었고 문단 길이·결론 위치·
    // 읽는 호흡에 대한 지시가 하나도 없었다. 그 결과 우리 원고 320편의 문단 길이 중앙값이
    // 122자(최대 377자)로 균질하게 굳었다. 같은 잣대로 잰 네이버 상위 노출 글 46편은 11자다.
    "",
    "읽는 사람 기준 (네이버는 모바일에서 읽는다):",
    "- 한 문단은 40자를 넘기지 않는다. 길어도 60자에서 끊는다. 문단 사이는 빈 줄로 띄운다.",
    "- 한 문단에는 하나만 말한다. 두 가지를 말해야 하면 문단을 나눈다.",
    "- 글의 결론은 세 번째 문단 안에 이미 나와 있어야 한다. 끝까지 읽어야 답이 나오는 구조로 쓰지 않는다.",
    "- 첫 다섯 문단(약 200자) 안에 독자가 찾던 답의 핵심이 들어간다.",
    "- 소제목은 400~500자마다 하나씩 둔다.",
    // 볼드: 상위 노출 글 46편 중 볼드를 쓰지 않은 글이 0편이었다(중앙값 14개, 1,000자당 9.3개).
    // 우리 글은 0개다. 문장 전체를 굵게 하면 훑는 눈이 내려앉을 지점이 사라진다.
    `- 핵심 어절을 **굵게** 표시한다. 1,000자당 8~10곳, 이 글 전체에서 ${Math.max(8, Math.round(yeriPayloadWordCount(payload) / 1000 * 9))}곳 안팎이다.`,
    "- 굵게 하는 단위는 문장 전체가 아니라 핵심 어절 2~6자다. 한 문단에 굵은 곳은 최대 하나다.",
    "- 굵게 표시는 숫자, 기준, 판단이 갈리는 말에 붙인다. 접속사나 수식어에 붙이지 않는다.",
    `- [이미지] 줄은 정확히 ${imageCount}개만 작성한다. 이미지가 0개면 만들지 않는다.`,
    // 2026-08-25 이전에는 "관련 문단 뒤에 분산 배치"만 지시했다. 그 결과 이미지가 글자 수를
    // 채우듯 흩뿌려져 무엇을 보여주는 사진인지 알 수 없는 자리에 들어갔다.
    // 규칙을 "무엇을 뒷받침하는가"로 바꾼다 — 뒷받침할 내용이 없으면 넣지 않는 것이 낫다.
    "- 각 [이미지] 는 바로 앞 문단이 말한 내용을 눈으로 보여주는 사진이어야 한다. 앞 문단과 관계없는 장면을 넣지 않는다.",
    "- [이미지] 는 그 내용을 설명한 문단이 끝난 바로 다음 줄에 놓는다.",
    "- 소제목 바로 다음 줄에는 [이미지] 를 놓지 않는다. 소제목 아래 첫 설명 문단을 먼저 쓰고 그 뒤에 놓는다.",
    "- [이미지] 두 줄을 연달아 놓지 않는다. 이미지 사이에는 반드시 본문 문단이 하나 이상 있어야 한다.",
    "- 글을 맺는 마지막 문단 안이나 그 뒤에는 [이미지] 를 놓지 않는다.",
    ...
    ...(yeriImageModelRendersText(payload)
      ? [
        // 2026-08-18 실측(CEO + 자체 검증): gpt-image-2 는 짧은 한글을 정확히 그린다
        // ("오늘 쿠키 완판", "주문 마감 6시" 자소 오류 0). 사용자는 이미지 프롬프트를
        // 직접 입력하지 못하고 예리가 본문을 쓰며 만든다 — 그래서 "글자가 필요한가"의
        // 판단 기준을 여기 지침으로만 줄 수 있다. 허용에 그치면 실제로는 거의 안 넣는다.
        "- 이 작업의 이미지 모델은 한글을 정확히 그린다. 짧은 한글 문구가 내용 전달에 도움이 되는 장면이면 넣는다. 간판, 메모, 라벨, 가격표, 안내 문구처럼 한 줄이면 충분한 곳이 대상이다.",
        "- 문구는 한 줄, 10자 안팎으로 짧게 쓰고 한 이미지에 한 곳만 넣는다. 예: '주방 노트에 「주문 마감 6시」라고 적힌 메모'.",
        "- 글자가 없어도 뜻이 통하는 장면에는 억지로 넣지 않는다. 화면 가득한 문장, 긴 목록, 채팅 로그, 앱 화면 전체처럼 글자가 많은 구도는 여전히 피한다.",
      ]
      : [
    ...
    "- 배치할 자리가 규칙에 맞지 않으면 이미지 수를 채우려고 억지로 넣지 말고, 그 이미지를 쓸 수 있는 설명 문단을 먼저 만든다.",
    "- 제목은 content_markdown의 첫 줄에도 '# 제목' 형식으로 넣는다.",
    // 2026-08-25: 실행기가 목록·표·사진 설명을 넣을 수 있게 됐다. 그전에는 이 형식을
    // 만들어도 전부 평범한 문단으로 들어가서 지시할 이유가 없었다.
    "- 순서가 있는 절차는 '1. ' 로, 순서가 없는 확인 항목은 '- ' 로 시작하는 목록으로 쓴다. 목록 항목은 한 줄로 짧게 쓴다.",
    "- 목록으로 쓸 내용을 문단 안에 쉼표로 나열하지 않는다. 항목이 세 개 이상이면 목록으로 쓴다.",
    "- 비교 기준이 두 가지 이상이고 비교 대상이 두 개 이상일 때만 표를 쓴다. 그 외에는 표를 쓰지 않는다.",
    "- 표를 쓸 때는 반드시 3열, 머리행 포함 3행으로 만든다. '| 항목 | 항목 | 항목 |' 형식으로 쓰고 머리행 다음 줄에 '| --- | --- | --- |' 를 넣는다.",
    "- 표는 글 전체에서 최대 1개만 쓴다.",
    "- [이미지] 줄 뒤에 ' || ' 를 붙이고 사진 설명을 한 문장으로 쓴다. 예: '[이미지] 하부를 점검하는 장면 || 하부 부식은 육안으로도 확인됩니다'.",
    "- 사진 설명은 사진에 보이는 것이 왜 중요한지 알려주는 문장으로 쓴다. 앞 문단을 그대로 반복하지 않는다.",
...
const CAFE24_NON_STAFF_PRODUCT_PATTERNS = [
  ...
  /무료배포판|무료배포/,
  // 라운지 전자책은 AIMAX 계정이 필요 없는 상품인데 무시 목록에 없어서 주문마다
  // "매핑 불가" 알림이 떴다. 2026-08-20 실측: 277·284·286·321 전부 알림 대상이었다.
  // 321 은 상품명에 'AI 직원' 이 들어가 직원 상품처럼 보이기까지 한다
  // ("[예약판매] 내 생각을 영상으로 만드는 AI 직원 — 노트 한 장에서 유튜브 업로드까지…").
  // 무시 판정이 직원 규칙보다 먼저 돌기 때문에 패턴은 반드시 좁게 쓴다.
  /전자책|스레드가이드북|컨셉의정석|ai에게시켰는데|내생각을영상으로만드는ai직원|노트한장에서유튜브업로드/,
  // 강의·모임·웨비나도 AIMAX 계정이 필요 없는 상품인데 무시 목록에 없었다.
  // 2026-08-20 실측: 293 무료특강 · 100/101/253/133 패밀리데이 · 285 8주 클래스 ·
  // 279/296 무료웨비나 · 107 창업 프로그램 5기 전부 "매핑 불가" 알림 대상이었다.
...
function reportActionChecklist(row) {
  ...
  if (String(row?.auto_guidance_category || "") === "core_exe_missing") {
    return [
      "키보드에서 윈도우키를 누른 채 R 을 누르면 작은 창이 뜹니다.",
      "그 창에 %LOCALAPPDATA%\\Programs\\AIMAX 를 붙여넣고 확인을 누르면 AIMAX 설치 폴더가 열립니다.",
      "그 폴더에 AIMAX 실행 파일이 보이는지 확인합니다. 보이면 원인이 다른 데 있으니 5번으로 알려주세요.",
      "보이지 않으면, 쓰시는 백신(윈도우 보안, V3, 알약 등)에서 차단하거나 지운 기록에 AIMAX 가 있는지 보고 있으면 '허용'으로 되돌린 뒤, 설치 파일을 한 번 더 실행합니다.",
      "막히는 단계가 있으면 그 화면을 캡처해 아래 '아직 안 돼요'로 보내주세요.",
    ];
  }
...
const REPORT_AUTO_GUIDANCE = {
  ...
  // 본문은 서버 키로 생성되지만 이미지는 PC 실행기에 저장된 키만 사용한다.
  // 웹에만 키를 저장한 사용자는 "키를 넣었는데 왜 없다고 하냐"가 되므로,
  // 일반 api_key_missing 안내(웹 설정을 가리킴)와 반드시 분리해야 한다.
  image_local_key_missing: {
    status: "waiting_user",
    status_label: "사용자 확인 필요",
  ...
  // 사용자가 손댈 수 있는 게 없는 케이스. waiting_user 로 두면 사용자만 헛돌다가
  // 방치되므로 운영팀 확인 상태로 시작한다.
  ...
  core_exe_missing: {
    status: "waiting_user",
    status_label: "사용자 확인 필요",
    public_message: "AIMAX 프로그램 본체가 회원님 컴퓨터에서 없어진 상태입니다. 바로가기는 남아 있어서 아이콘은 눌리지만, 실행될 프로그램이 없어 창이 뜨지 않습니다. 저희 쪽 설치 파일은 확인해보니 정상이고, 회원님 계정이나 설정 문제도 아닙니다. 백신 프로그램이 파일을 지웠거나 설치가 끝까지 되지 않았을 때 이런 상태가 됩니다. 어느 쪽인지는 아래 한 가지만 확인해주시면 바로 알 수 있습니다.",
    next_update_message: "먼저 프로그램이 정말 없어졌는지만 확인해주세요. 키보드 윈도우키를 누른 채 R을 누르면 작은 창이 뜹니다. 거기에 %LOCALAPPDATA%\\Programs\\AIMAX 를 붙여넣고 확인을 누르면 AIMAX 설치 폴더가 열립니다. 그 안에 AIMAX 실행 파일이 보이지 않으면 백신이 지운 것이 맞습니다. 쓰시는 백신(윈도우 보안, V3, 알약 등)에서 차단하거나 지운 기록을 열어 AIMAX를 '허용'으로 되돌린 뒤 설치 파일을 한 번 더 실행해주세요. 어느 단계든 막히면 그 화면을 캡처해 '아직 안 돼요'로 보내주시면 저희가 이어서 안내드립니다.",
  },
...
function classifyReportAutoGuidance(report) {
  ...
  const rules = [
    ...
    ["editor_structure_changed", /editor_title_area_not_found|se-section-documenttitle|제목 입력 영역을 찾지 못/i],
    ["image_local_key_missing", /이미지 생성용 로컬 api 키/i],
...
function buildWaitingUserReportMail(row) {
  ...
  lines.push("");
  if (corrected) {
    lines.push("앞서 보내드린 안내가 이 증상과 맞지 않았습니다. 안내대로 하셨는데도 안 되셨다면 저희가 원인을 잘못 짚은 것입니다. 번거롭게 해드려 죄송합니다.");
    lines.push("다시 확인한 내용을 아래로 정정해 보내드립니다.");
  } else {
